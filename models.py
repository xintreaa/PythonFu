...
class BaseModel(metaclass=BaseModelMeta):
    ...
    def create(cls, **data):
        ...
        obj_id = cls._db_instance.create(cls.__name__.lower() + 's', data)
        return cls(id=obj_id, **data)

    @classmethod
    def get(cls, obj_id):
        """Отримує об'єкт за ID."""
        cls._ensure_db()  # Make sure we have a database
        obj_data = cls._db_instance.read(cls.__name__.lower() + 's', obj_id)
        if obj_data:
            return cls(**obj_data)
        return None
    ...
    def update(cls, obj_id, **data):
        ...
        cls._ensure_db()  # Make sure we have a database
        updated_rows = cls._db_instance.update(cls.__name__.lower() + 's', obj_id, data)
        if updated_rows:
            return cls(id=obj_id, **data)
        return None
    ...
    def delete(cls, obj_id):
        ...
        cls._ensure_db()
        return cls._db_instance.delete(cls.__name__.lower() + 's', obj_id)
